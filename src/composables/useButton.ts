...
import { type Props } from '@/components/storm-ui/IconButton.vue'
...
export function useButton (props: Props, variant: string = 'ghost') {
  ...
  const variants: KeyValue = {
    ...
    gdanger: 'cursor-pointer border border-transparent rounded text-center text-neutral-900 bg-transparent hover:bg-neutral-100 hover:text-red-600 hover:border-red-300 focus:outline-none focus:ring-2 focus:ring-red-200 active:!bg-red-50 focus:text-red-400 focus:border-red-400',
    gprimary: 'cursor-pointer shadow-sm border border-blue-400 rounded relative text-blue-500 hover:text-white bg-white hover:bg-blue-400  focus:border-white focus:outline-none focus:ring-2 ring-offset-white focus:ring-blue-500 active:bg-blue-700',
    iconbar: 'cursor-pointer border rounded border-transparent text-center text-gray-100 bg-transparent hover:text-yellow-500 focus:outline-none focus:ring-2 ring-offset-white focus:ring-blue-500 active:bg-neutral-900',
  ...
  const getVariant = computed(() => {
    let classes: string = props.disabled ? disable(variants[variant]) : variants[variant]
    ...
    if (props.active === true && props.variant === 'iconbar') {
      classes = classes.replace('bg-transparent', 'bg-gray-800')
      classes = classes.replace('text-white', 'text-yellow-500')
    }
